...
<link rel="icon" type="image/png"
	href="${pageContext.request.contextPath }/wmcr/images/favicon.ico" />
...
<link rel="stylesheet"
	href="${pageContext.request.contextPath }/wmcr/css/common.css?v=2015-5-20" />

<link rel="stylesheet"
	href="${pageContext.request.contextPath }/wmcr/css/checkout.css" />
...
<body class="day " ng-controller="bodyCtrl" day-or-night>
		...
		<header id="header">
			<div class="common-width clearfix">
				...
				<ul class="member logging" ng-init="loginInfo=true">
					...
					<li class="userName"><a href="/account/center/manage/"
						rel="nofollow" draw-user>18005151538<em></em></a>
						<div>
							<p>
								<a href="/account/center/manage/" rel="nofollow">账号管理</a>
							</p>
							<p>
								<a href="/account/center/address/" rel="nofollow">地址管理</a>
							</p>
							<p class="no-bo">
								<a id="logout" href="/account/logout/" referer-url
									rel="nofollow">退出</a>
							</p>
						</div></li>
					<li class=""><a href="/account/center/orders/"
						class="order-center" rel="nofollow">我的订单</a></li>
					<li class=""><a href="/account/center/favorite/"
						rel="nofollow">我的收藏</a></li>
					<li class=""><a href="/account/gift/center/" rel="nofollow">氪星礼品站</a></li>
					<li class="phone-client "><a href="/app/" rel="nofollow"
		...
		<div id="main-box">
			...
			<section class="checkout-main common-width">
				...
				<section class="user-info">
					<ul class="user-address-list clearfix disnone"
						ng-class="{disblock: userAddressList.length != 0}">
						<li ng-repeat="item in userAddressList"
							ng-class="{active:item.active,userAddressHover:mobileAny}"
							ng-click="changeActiveAddress($index)"
							class="user-address-item fl">
							<div class="clearfix">
 ...
 -->
						...
						<li
							ng-if="userAddressList.length > 0 && userAddressList.length < 8"
							class="user-address-item address-add-box fl">
							<div ng-click="addUserAddress()">
								<i>+</i>使用新地址
							</div>
						</li>
					</ul>
					<form novalidate="true" name="orderForm" class="order-form inline">
						<div ng-show="userAddressList.length == 0">
							<div class="form-group row mb10">
								...
								<div class="col-8">
									<input type="text" required ng-blur="orderForm.name.blur=true"
										ng-focus="orderForm.name.blur=false" maxlength="10"
										ng-class="{error:(orderForm.submit || orderForm.name.blur) && orderForm.name.$invalid}"
										name="name" placeholder="您的称呼" ng-model="name">
								</div>
								<div class="col-8 col-offset-2 disnone"
									ng-class="{disnone : !((orderForm.submit && orderForm.name.$invalid) || (!orderForm.submit && orderForm.name.blur && orderForm.name.$invalid))}">
							...
							<div class="form-group row mb10">
								...
								<div class="col-8">
									<input type="text" maxlength="11"
										ng-blur="orderForm.contact.blur=true"
										ng-focus="orderForm.contact.blur=false" required mobile
										ng-class="{error:(orderForm.submit || orderForm.contact.blur) && orderForm.contact.$invalid}"
										name="contact" placeholder="您的联系方式" ng-model="phone">
								</div>
								<div class="col-8 col-offset-2 disnone"
									ng-class="{disnone: !((orderForm.submit || orderForm.contact.blur) && orderForm.contact.$error.required)}">
									<span class="vaildate-error">联系方式不能为空</span>
								</div>
								<div class="col-8 col-offset-2 disnone"
									ng-class="{disnone: !(orderForm.contact.blur && orderForm.contact.$error.mobile&&!orderForm.contact.$error.required)}">
							...
							<div class="form-group row mb10">
								...
								<div class="col-8">
									<input type="text" required
										ng-blur="orderForm.address.blur=true"
										ng-focus="orderForm.address.blur=false"
										ng-class="{error:(orderForm.submit || orderForm.address.blur) && orderForm.address.$invalid}"
										name="address" placeholder="详细地址，如武定西路1189号606室"
										ng-model="address">
								</div>
								<div class="col-8 col-offset-2 disnone"
									ng-class="{disnone: !((orderForm.submit && orderForm.address.$invalid) || (!orderForm.submit && orderForm.address.blur && orderForm.address.$invalid))}">
						...
						<div class="form-group row mb10">
							...
							<dh-select class="col-8" data="selectObj"
								selectedindex="datetimeIndex"></dh-select>
						</div>
						<div class="form-group row mb10 relative">
							<label class="col-2">支付方式：</label>

							<dh-radio class="col-2" model="payType" value="0" title="在线支付"></dh-radio>
						...
						<div class="form-group row mb10">
							...
							<div class="col-8">
								<input type="text" maxlength="150" placeholder="如：多米饭，不吃辣等口味需求"
									ng-model="comment">
							</div>
						</div>
					</form>
				...
				<section>
					...
					<div class="order-info">
						...
						<div class="order-body">
							<c:forEach items="${detailsList }" var="v">
								<div class="order-item clearfix">
									<div class="goods-name">${v.foodName }</div>
									<div class="goods-count">${v.num }</div>
									<div class="goods-price">￥${v.price }</div>
									<div class="goods-total">￥${(v.price*v.num) }</div>
								</div>
							</c:forEach>
 ...
 -->
				...
				<section class="total-sum">
					...
					<p class="tr fs17 pink">
						需要付款： <b>￥${ord.money }<span ng-init="orderTotal=32.00"
							ng-bind="orderTotal|number:2"></span></b>
					</p>
					<p class="tr last">
						<a
						...
						<button
							ng-disabled="!(name && phone && address&&couponCheck&&commitCheck)"
							ng-click="commitOrder()" class="btn btn-success fs20">
							提交订单 <i class="icon arrows-right"></i>
						</button>
					</p>
				</section>
			</section>
		...
		<div class="footer-first gray">
			<div class="company-info clearfix fs14 gray">
				...
				<a href="/jobs/" target="_blank" rel="nofollow">人才招聘</a> <a
					href="/contact/" target="_blank" rel="nofollow">联系我们</a> <a
					href="javascript:;" user-feedback ng-click="userFeedback=true"
					class="last" rel="nofollow">意见反馈</a> <a
					href="/restaurant/list/shanghai/1/" class="last" target="_blank"
					style="display: none">上海餐厅导航</a>
			</div>
		</div>
		<div class="footer-last">
			<a target="_blank" class="foot-logo-1 base-logo" href="index.html"></a>
			<div class="tc fs14 light-gray mb10">©2014 waimaichaoren.com
				All Rights Reserved - 沪ICP备11019908号</div>
		</div>
	</footer>
	...
	<div class="form-group row mb20 clearfix">
		<div class="fl w185">
			<input type="text" ng-class="{error:msg.captchaMessage}"
				ng-focus="msg.captchaMessage=''" placeholder="请输入验证码"
				ng-model="msg.captcha" maxlength="6" /> <span
				class="vaildate-error" ng-if="msg.captchaMessage"> <span
				ng-bind="msg.captchaMessage"></span>
			</span>
		</div>
		<button class="fs12 fr w40p btn btn-pink" ng-click="getMsgCaptcha()"
			ng-disabled="msgCaptchaDisabled" ng-bind="msgCaptchaVal"></button>
	</div>
	<div class="errorBtn">
		<button class="btn btn-success normal-btn" ng-click="submitMsg()"
			ng-disabled="msg.captcha.length != 6">确认</button>
	</div>
	</dh-dialog>
	...
	<div class="coupon-confirm">
		...
		<button class="btn btn-success couponConfirm-btn"
			ng-click="submitcouponConfirm()">不使用优惠劵直接下单</button>
	</div>
	</dh-dialog>
	<dh-dialog class="disnone" header="使用新地址/修改地址" width="420"
	...
	<div ng-controller="userAddressCtrl">
		<form novalidate="true" name="userAddressForm" class="address-from">
			<div class="form-group row mb20">
				...
				<div class="col-8">
					<input type="text" required maxlength="10"
						ng-class="{error:userAddressForm.submit && userAddressForm.name.$invalid}"
						id="adsName" name="name" placeholder="您的称呼"
						ng-model="userAddress.customer_name">
				</div>
				<div class="col-8 col-offset-3"
					ng-if="userAddressForm.submit && userAddressForm.name.$invalid">
					<span class="vaildate-error">称呼不能为空</span>
				</div>
			</div>
			<div class="form-group row mb20">
				...
				<div class="col-8">
					<input type="text" maxlength="11" required mobile
						ng-class="{error:userAddressForm.submit && userAddressForm.contact.$invalid}"
						id="adsPhone" name="contact" placeholder="您的联系方式"
						ng-model="userAddress.customer_phone">
				</div>
				<div class="col-8 col-offset-3"
					ng-if="userAddressForm.submit && userAddressForm.contact.$error.required">
					<span class="vaildate-error">联系方式不能为空</span>
				</div>
				<div class="col-8 col-offset-3"
					ng-if="userAddressForm.submit && !userAddressForm.contact.$error.required && userAddressForm.contact.$error.mobile">
					<span class="vaildate-error">请输入正确的11位手机号码</span>
				</div>
			</div>
			<div class="form-group row mb30">
				...
				<div class="col-8">
					<input type="text" required
						ng-class="{error:userAddressForm.submit && userAddressForm.address.$invalid}"
						id="adsAds" name="address" placeholder="详细地址，如武定西路1189号606室"
						ng-model="userAddress.delivery_address">
				</div>
				<div class="col-8 col-offset-3"
					ng-if="userAddressForm.submit && userAddressForm.address.$invalid">
					<span class="vaildate-error">送餐地址不能为空</span>
				</div>
			</div>
			<!-- 用户地址提交 -->
			<div class="form-group tc">
				<button class="btn btn-success normal-btn" id="doAddressSubmit">确认</button>
				<button class="btn btn-cancel normal-btn"
					ng-click="cancelUserAddress()">取消</button>
			</div>
		</form>
	</div>
	</dh-dialog>
	...
	<ul class="site-fixed">
		<li class="scroll-top"><img
			src="${pageContext.request.contextPath }/wmcr/images/scroll_top1.png"
			alt="" /></li>
		<li class="scorll-feekback" ng-click="userFeedback=true"><img
			src="${pageContext.request.contextPath }/wmcr/images/scorll_feekback.png"
			alt="" />
			<div>意见反馈</div></li>
		<li class="scroll-wx"><img
			src="${pageContext.request.contextPath }/wmcr/images/scroll_wx.png"
			alt="" />
			<div>
				<img src="${pageContext.request.contextPath }/wmcr/images/wx.png"
					alt="" />
			</div></li>
	</ul>

	<script type="text/javascript"
		src="${pageContext.request.contextPath }/wmcr/js/angular.min.js"></script>
	...
	<script>
		(function() {
			...
			var a = 0;
			var e = /www[.]google-analytics[.]com/i;
			var h;
			function d(j, k, i) {
				if (j.addEventListener) {
					j.addEventListener(k, i, false)
				} else {
					if (j.attachEvent) {
			...
			function g(j, k, i) {
				if (j.removeEventListener) {
					j.removeEventListener(k, i, false)
				} else {
					if (j.detachEvent) {
			...
			function c(j, l) {
				...
				var k = window.MutationObserver
						|| window.WebKitMutationObserver || false;
				if (!!k) {
					j._observer = new k(function(m) {
						m.forEach(function(n) {
							if (n.attributeName == "src"
									&& e.test(n.target.src)) {
								i(n.target)
							}
						})
					});
					j._observer.observe(j, {
						attributes : true,
						childList : false,
						characterData : false
					})
					...
					if (j.addEventListener) {
						j._DOMAttrModified = function(m) {
							if (m.attrName == "src" && e.test(m.target.src)) {
								i(m.target)
							}
						};
						j.addEventListener("DOMAttrModified",
								j._DOMAttrModified)
					} else {
						if (j.attachEvent) {
							j._onpropertychange = function(m) {
								if (m.propertyName == "src"
										&& e.test(m.srcElement.src)) {
									i(m.srcElement)
								}
							};
							j.attachEvent("onpropertychange",
									j._onpropertychange)
						}
					}
				}
			...
			function f(i, k) {
				var j = window.MutationObserver
						|| window.WebKitMutationObserver || false;
				if (!!j) {
					i._observer.disconnect();
					delete i._observer
				} else {
					if (i.addEventListener) {
						i.removeEventListener("DOMAttrModified",
								i._DOMAttrModified);
						delete i._DOMAttrModified
					} else {
						if (i.attachEvent) {
							i.detachEvent("onpropertychange",
									i._onpropertychange);
							delete i._onpropertychange
						}
					}
				}
				g(i, "error", k);
			...
			document.createElement = function(i) {
				element = b.call(document, i);
				if (i.toLowerCase() == "img") {
					var j = function(k) {
						f(k.target || k.srcElement, j)
					};
					c(element, j)
				}
				return element
			};
			this.ga_synchronize = function(i) {
				h = i
			}
		}());
	</script>
