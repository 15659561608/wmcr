...
<body>
	<nav class="breadcrumb">
		...
		门店管理 <span class="c-gray en">&gt;</span> 门店列表 <a
	...
	<div class="page-container">
		...
		<span>输入门店名称检索：<input type="text" class="form-controlSearch input-text "
				placeholder="输入门店名称检索" data-column="4" id="col4_filter"
				style="width: 200px;"></span>
			<span class="r">共有数据：<strong><span
	...
	<script>
		$(function() {
			...
			//修改员工信息的超链接单击事件
			 $(document).on("click",'.editSal',function()
			 {
				...
				/*  
			     ...
			     layer.open({
			 		...
			 		shade:0.4,
			 		title: '编辑促销信息', //显示的标题
			 		content: 'updateSales.jsp', //很多种写法 其中之一直接写目标窗口(要弹出来窗口)
	...
	<script>
		...
		employee.requestUrl = {
			queryList : employee.url + "wmcr/SalesServlet" //数据是从servlet一侧返回的 json格式
		};
		...
		employee.filed = [
				{ //第一个列
					...
					"createdCell" : function(nTd, sData, oData, iRow, iCol) {
						$(nTd)
								.html(
										"<input type='checkbox' value='"+oData.salId+"' name='checkList' value='" + sData + "'>");
					}
				...
				{ //创建操作那个列
					...
					"createdCell" : function(nTd, sData, oData, iRow, iCol) {
						...
						$(nTd).html(
										' <a title="删除" onclick="delSales(this)" href="${pageContext.request.contextPath}/SalesServlet?op=del&id='+oData.salId+'" class="del ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>&nbsp;<a title="编辑" onclick="showUpdate(this)"  href="javascript:;" class="editSal ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe60c;</i></a> ');
	...
	function showUpdate(obj){
		...
		layer.open({
	 		...
	 		shade:0.4,
	 		title: '修改促销信息',
	 		content: '../SalesServlet?op=updateInfo&id='+id,
